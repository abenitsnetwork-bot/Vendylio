// Phase 10 — backfills test coverage for POST /api/admin/withdrawals/[id]/cancel.
// Discovered missing while building the admin UI that exercises it — see
// users/[id]/role/route.test.ts's header comment for context.
...
function makeReq(body: unknown, csrf: 'match' | 'missing' = 'match'): NextRequest {
  ...
  return new NextRequest('http://test/api/admin/withdrawals/w1/cancel', {
    method: 'POST',
    headers,
...
describe('POST /api/admin/withdrawals/[id]/cancel', () => {
  it('403s when CSRF header is missing', async () => {
    const res = await POST(makeReq({ reason: 'fraud' }, 'missing'), ctxWith('w1'));
    expect(res.status).toBe(403);
    expect(mockRequireSuperadmin).not.toHaveBeenCalled();
  });

  it('propagates 403 from requireSuperadmin (ADMIN cannot cancel)', async () => {
    mockRequireSuperadmin.mockResolvedValueOnce(
      NextResponse.json({ error: 'ADMIN_REQUIRED' }, { status: 403 }),
    );
    const res = await POST(makeReq({ reason: 'fraud' }), ctxWith('w1'));
    expect(res.status).toBe(403);
  });
  ...
  it('404s WITHDRAWAL_NOT_FOUND when the owner lookup misses (before the lock)', async () => {
    prismaMock.withdrawal.findUnique.mockResolvedValueOnce(null as never);
    const res = await POST(makeReq({ reason: 'fraud' }), ctxWith('missing'));
  ...
  it('acquires the advisory lock on the WITHDRAWAL OWNER (not the admin actor) as the first tx statement', async () => {
    ...
    prismaMock.withdrawal.findUnique
      .mockResolvedValueOnce({ userId: 'seller_owner' } as never) // phase 1: owner lookup
      .mockResolvedValueOnce(w as never); // phase 2: re-fetch under lock
    prismaMock.withdrawal.update.mockResolvedValueOnce({ ...w, status: 'CANCELLED' } as never);

    await POST(makeReq({ reason: 'fraud' }), ctxWith('w1'));
  ...
  it.each(['COMPLETED', 'FAILED', 'CANCELLED'] as const)(
    '409s WITHDRAWAL_NOT_CANCELLABLE when status is %s',
    async (status) => {
  ...
  it.each(['PENDING', 'PROCESSING'] as const)(
    'cancels a %s withdrawal and logs the action',
    async (status) => {
      ...
      const res = await POST(makeReq({ reason: 'Refunded via bank' }), ctxWith('w1'));
      ...
      expect(updateArg?.data).toMatchObject({
        status: 'CANCELLED',
        failureReason: 'Refunded via bank',
      });

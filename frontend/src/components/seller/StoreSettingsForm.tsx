...
export function StoreSettingsForm({
  ...
  const isDirty =
    ...
    heroSubhead !== (store.heroSubhead ?? '') ||
    announcement !== (store.announcement ?? '') ||
    heroImages.join(' ') !== parseHeroImages(store.heroImages).join(' ');
      ...
      <Card className="p-5 sm:p-8">
        <div className="mb-6 border-b border-border pb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            A rotating banner at the top of your storefront — up to 3 photos and one promo message.
          </p>
        </div>
        <div className="space-y-6">
          <Field label="Announcement strip" htmlFor="announcement">
            <input
              id="announcement"
              type="text"
              maxLength={140}
            ...
            />
            <p className="mt-1 text-xs text-muted-foreground">
              A thin bar shown at the very top of every storefront page. Leave empty to hide it.
            </p>
          </Field>
          <StoreHeroEditor
            images={heroImages}

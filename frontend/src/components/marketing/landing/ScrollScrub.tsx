...
// Scroll-scrubbed hero video engine — ported from the design reference
// (design-reference/vendylio-landing/app/src/components/scroll-scrub). Self
// contained: its own rAF loop reads native window scroll and drives
// `<video>.currentTime` directly, independent of the page's GSAP/Lenis smooth
// scroll (see LandingMotion.tsx). Loads the clip via `fetch` + a Blob URL
// (not a bare `src=`) so it's fully buffered before scrubbing and seeking is
// frame-accurate; the blob is revoked and the <video> element torn down on
// unmount or on a responsive source swap. Bails out entirely under
// `prefers-reduced-motion: reduce` — the poster stays visible, no fetch, no
// rAF loop.

...
- **`/api/stores/domain`** — domaine personnalisé (Pro) : `POST`/`GET`/`DELETE`, via l'API Vercel Domains ; le middleware réécrit `Host: shop.marque.com` → `/s/<slug>` (inerte sans `VERCEL_API_TOKEN`)

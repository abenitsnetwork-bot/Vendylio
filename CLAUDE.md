...
**Cron strategy.** No `setInterval` loops — Next.js / Vercel doesn't keep long-lived processes. Background work runs as **Vercel Cron** routes under `app/api/cron/<name>/route.ts`, each gated by `Authorization: Bearer ${CRON_SECRET}`. Targets: `outbox-drain` (1m), `email-queue-drain` (1m), `verification-cleanup` (hourly), `order-expiration` (5m), `webhook-log-purge` (daily), `email-job-purge` (daily — purges SENT EmailJob rows older than `EMAIL_JOB_RETENTION_DAYS`, default 30), `low-stock-sweep` (daily — safety net for the low-stock alerts: enqueues `notification.low_stock` / `notification.out_of_stock` for products/variants at or below their effective threshold whose `lowStockNotifiedAt` is still null; the on-sale path in `orders/markPaid.ts` covers the common case), `fulfillment-tick` (2m — Prompt #12: dispatches PENDING courier deliveries for READY orders, polls in-flight deliveries for intermediate/missed-terminal courier states, purges stale `Quote` rows). Requires a **Vercel Pro** plan or higher — Hobby only allows once-daily cron schedules, which is too slow for the 15-minute verification-code TTL. Multi-instance coordination still uses [frontend/src/lib/server/leader-lease.ts](frontend/src/lib/server/leader-lease.ts) Redis leases where two crons could collide. The Bictorys charge `CircuitBreaker` is still in-memory single-instance — replace with a Redis-backed variant for multi-pod prod (documented limitation).
...
- `frontend/src/lib/server/cron/` — extend with `verifyCronSecret(req)` consumers; add new cron route handlers under `frontend/src/app/api/cron/<name>/route.ts` mirroring the 7 existing crons; ALL cron handlers must verify `Authorization: Bearer ${CRON_SECRET}` via the shared `verifyCronSecret` helper

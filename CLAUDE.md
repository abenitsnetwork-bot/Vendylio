...
**Storefront hero (Phase 9).** `Store` gains `heroImages` (JSON — ordered array of up to 3 uploaded image URLs, capped/sanitized by `parseHeroImages` in [frontend/src/lib/storeHero.ts](frontend/src/lib/storeHero.ts)) + `heroHeadline` / `heroSubhead` (one global promo message). [frontend/src/components/storefront/StorefrontHero.tsx](frontend/src/components/storefront/StorefrontHero.tsx) renders an auto-advancing, dependency-free carousel above the product list in all 3 templates; empty `heroImages` → renders nothing (storefront unchanged). Edited via the shared [frontend/src/components/seller/StoreHeroEditor.tsx](frontend/src/components/seller/StoreHeroEditor.tsx) in onboarding (`/onboarding/brand`) and Settings → Store; images reuse the existing `/api/upload` (Cloudinary) + `ImageDropzone`. Validated on `PATCH /api/stores` (`heroImages` ≤ 3 URLs).
